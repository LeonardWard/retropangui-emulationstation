...
public:
	...
	// RetroPangui: 이 함수는 반드시 "부모가 이미 window의 GUI 스택에 올라간
	// 뒤"에만 호출해야 함 - 예: GuiWifiSelect처럼 "화면 들어오자마자 팝업
	// 자동 표시"를 하고 싶으면, 부모 생성자 안이 아니라 pushGui(부모) 호출
	// 직후 별도로 호출할 것. 부모 생성자 안에서 부르면 그 시점엔 부모
	// 자신이 아직 스택에 없는 상태라(부모 생성자가 다 끝나야 바깥의
	// pushGui(new 부모(...))가 실행됨) 팝업이 부모보다 먼저 스택에 들어가
	// [..., 팝업, 부모] 순으로 뒤집힘 → 팝업이 부모 밑에 깔려 화면에 안
	// 보이고 입력도 안 받다가, 나중에 부모가 닫히는 순간 뒤늦게 드러나서
	// 이미 소멸된 부모/자신을 참조해 크래시함(2026-07-25 실기기 코어덤프로
	// 확정 - WiFi 네트워크 재선택 시 재현). update() 프레임으로 미루는
	// 방식도 시도해봤으나 ComponentList가 커서가 가있는 행만 업데이트해서
	// "커서를 옮겨야 팝업이 뒤늦게 뜨는" 새 문제가 생겨 폐기.
	void openPopup() { open(); }
