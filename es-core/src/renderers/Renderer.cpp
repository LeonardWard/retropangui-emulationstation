...
//////////////////////////////////////////////////////////////////////////
	...
	static bool getRealDisplaySize(int& outW, int& outH)
	{
		...
		{
			// odroid-drm-fbset이 -getcurrentmode를 모르는 구버전이거나(그럴 땐
			// usage 메시지 후 exit 1), 지금 물리 링크가 안 잡힌 상태 등 -
			// 어느 쪽이든 이 값을 못 믿으므로 SDL 값을 그대로 쓰게 실패 반환.
			LOG(LogWarning) << "getRealDisplaySize: 조회 실패(exitStatus=" << exitStatus << ") - SDL 값 그대로 사용";
			return false;
		}

		// odroid-drm-fbset -getcurrentmode의 출력 포맷: "너비 높이 새로고침율 이름"
		int w = 0, h = 0, refresh = 0;
		if(sscanf(buf, "%d %d %d", &w, &h, &refresh) != 3 || w <= 0 || h <= 0)
			return false;
